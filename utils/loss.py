...
class DiceLoss(nn.Module):
    def __init__(self, from_logits=True, ignore_index=None):
        super().__init__()
        self.loss = smp.losses.DiceLoss(mode='multiclass', from_logits=from_logits, ignore_index=ignore_index)
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, ignore_index=None):
        super().__init__()
        self.loss = smp.losses.FocalLoss(mode='multiclass', alpha=alpha, gamma=gamma, ignore_index=ignore_index)
...
class ComboLoss(nn.Module):
    ...
    def __init__(self, dice_weight=0.5, ce_weight=0.5, from_logits=True, ignore_index=None):
        super().__init__()
        self.dice = DiceLoss(from_logits=from_logits, ignore_index=ignore_index)
