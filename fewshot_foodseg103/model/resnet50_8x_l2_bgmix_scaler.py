...
class ProtoNet(nn.Module):
    def __init__(self, n_way, alpha_bg=0.75, beta_mix=0.5):
        """
        ...
        # 全局先验背景原型
        self.gl_bg_proto = nn.Parameter(torch.zeros(1, feat_dim))  # (1, 2048)
    ...
    def forward(self, support_set, query_set, selected_classes):
        ...
        # 背景原型
        if self.beta_mix == 0:  # 此时跳过episode背景原型计算提升效率
            bg_proto = l2_normalize(self.gl_bg_proto)
        elif self.beta_mix == 1:
            bg_proto = compute_bg_prototypes(s_feat, s_masks, s_labels, selected_classes)
        else:
            ep_bg_proto = compute_bg_prototypes(s_feat, s_masks, s_labels, selected_classes)
            bg_proto = self.beta_mix * ep_bg_proto + (1 - self.beta_mix) * l2_normalize(self.gl_bg_proto)
