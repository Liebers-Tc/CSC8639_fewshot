...
class Visualizer:
    ...
    def pred_image(self, mask):
        """将 mask 转换为彩色图像用于可视化"""
        mask = mask.cpu().numpy()
        return plt.get_cmap('tab20')(mask % 20)[..., :3]  # RGB mask

    def overlay_image(self, image, mask, alpha=0.5):
        """将预测 mask 叠加到原图上，生成混合可视化图像"""
        image = self.denormalize(image).cpu()
        image_np = image.permute(1, 2, 0).numpy()
        mask_rgb = self.pred_image(mask)
        return ((1 - alpha) * image_np + alpha * mask_rgb).clip(0, 1)
    ...
    def group_image(self, image, gt_mask, pred_mask):
        ...
        axes[2].imshow(gt_mask.cpu(), cmap='tab20')
    ...
    def save_pred(self, preds, start_index=0):
        ...
        for i in range(total):
            ...
            save_path = os.path.join(self.save_dir, f"pred_{start_index + i + 1}.png")
            plt.imsave(save_path, pred_vis)
            if self.wandb_logger and self.upload_pred:
                self.wandb_logger.log({f"Pred_Img/Prediction_Img/{os.path.basename(save_path)}": self.wandb_logger.image(save_path)})
    ...
    def save_overlay(self, images, preds, start_index=0):
        ...
        for i in range(total):
            ...
            save_path = os.path.join(self.save_dir, f"overlay_{start_index + i + 1}.png")
            plt.imsave(save_path, overlay)
            if self.wandb_logger and self.upload_overlay:
                self.wandb_logger.log({f"Pred_Img/Overlay_Img/{os.path.basename(save_path)}": self.wandb_logger.image(save_path)})
    ...
    def save_group(self, images, gts, preds, start_index=0):
        ...
        for i in range(total):
            ...
            save_path = os.path.join(self.save_dir, f"group_{start_index + i + 1}.png")
            fig.savefig(save_path)
            if self.wandb_logger and self.upload_group:
                self.wandb_logger.log({f"Pred_Img/Group_Img/{os.path.basename(save_path)}": self.wandb_logger.image(save_path)})
            plt.close(fig)
