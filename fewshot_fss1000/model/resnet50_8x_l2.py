...
def compute_prototypes(feats, masks, labels, classes):
    ...
    return l2_normalize(torch.stack(prototypes))

# ProtoNet
# --------------------------------------------------
class ProtoNet(nn.Module):
    def __init__(self, n_way, pretrained_backbone=True):
        super().__init__()
        self.n_way = n_way
        self.encoder, _, _ = encoder(pretrained_backbone)
        self.decoder = decoder(n_way)
    ...
    def forward(self, support_set, query_set, selected_classes):
        ...
        # prototypes
        prototypes = compute_prototypes(s_feat, s_masks, s_labels, selected_classes)

        # similarity
        sims = torch.einsum('bchw,nc->bnhw', q_feat, prototypes)
