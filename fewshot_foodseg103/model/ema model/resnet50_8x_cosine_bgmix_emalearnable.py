...
class ProtoNet(nn.Module):
    def __init__(self, n_way, beta_mix=0.5, ema_rate=0.05):
        """
        ...
        # 全局先验背景原型
        self.gl_bg_proto = nn.Parameter(torch.empty(1, feat_dim))
    ...
    def forward(self, support_set, query_set, selected_classes):
        ...
        # ③ 在 no_grad 块里做 EMA：把 ep_bg 写回 gl_bg
        with torch.no_grad():
            # gl = (1-α)*gl + α*ep_bg
            self.gl_bg_proto.mul_(1 - self.ema_rate).add_(self.ema_rate * ep_bg_proto)
            # 余弦度量必须保持单位模长
